...
module RubyBeautify
  ...
  OPEN_BLOCK_START = ["module", "class", "begin", "def", 'if', 'while', 'unless', 'case'].freeze
  BOTH_BLOCK = ["else", "elsif", 'rescue', 'when'].freeze
  OPEN_BLOCK_DO = ['do', '{'].freeze
  CLOSE_BLOCK = ['end', '}'].freeze

  OPEN_BRACKETS  = %i[on_lparen on_lbracket on_lbrace on_embexpr_beg on_tlambeg].freeze
  CLOSE_BRACKETS = %i[on_rparen on_rbracket on_rbrace on_embexpr_end].freeze
  NEW_LINES = %i[on_nl on_ignored_nl on_comment on_embdoc_end].freeze
  ...
  def pretty_string(content, indent_token: "\t", indent_count: 1)
    ...
    lex.each do |token|
      line_lex << token
      next unless NEW_LINES.include? token[1] # if type of this token is a new line
      ...
      line_string = line_lex.map(&:last).join
  ...
  def starts_block?(line_lex)
    ...
    line_lex.each do |x|
      ...
      return x[1] == :on_kw && OPEN_BLOCK_START.include?(x[2]) if x[1] != :on_sp
    end
  end
  ...
  def both_block?(line_lex)
    line_lex.each do |x|
      # search for a first non-space token
      return x[1] == :on_kw && BOTH_BLOCK.include?(x[2]) if x[1] != :on_sp
    end
  end
